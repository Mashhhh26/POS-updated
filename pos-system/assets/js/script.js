...
(function(){
  ...
  function initSidebar(){
    ...
    function syncToggle(){
      ...
      const open=desktop ? !body.classList.contains('sidebar-collapsed') : body.classList.contains('sidebar-mobile-open');
    ...
    toggle.addEventListener('click',function(){
      if(desktopQuery.matches){
        ...
        try{localStorage.setItem('pos_sidebar_collapsed',body.classList.contains('sidebar-collapsed')?'1':'0');}catch(e){}
      }else{
    ...
    sidebar.querySelectorAll('.sidebar-section-toggle[data-sidebar-section]').forEach(function(btn){
      btn.addEventListener('click',function(){
        const id=btn.getAttribute('data-sidebar-section');
    ...
    try{
      if(desktopQuery.matches && localStorage.getItem('pos_sidebar_collapsed')==='1') body.classList.add('sidebar-collapsed');
    }catch(e){}

    function viewportChanged(){
      if(desktopQuery.matches) body.classList.remove('sidebar-mobile-open');
      else body.classList.remove('sidebar-collapsed');
      syncToggle();
    }
    if(desktopQuery.addEventListener) desktopQuery.addEventListener('change',viewportChanged);
    else if(desktopQuery.addListener) desktopQuery.addListener(viewportChanged);
    syncToggle();
  }
