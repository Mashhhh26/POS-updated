...
CREATE TABLE `purchase_orders` (
  ...
  `status` enum('draft','pending_approval','approved','partially_received','sent','acknowledged','fulfilled','closed','cancelled') DEFAULT 'draft',
...
-- Removed invalid orphan negative test row so FK constraints can be imported safely.
...
-- ENHANCEMENT MIGRATION: HRM accounts, archive, procurement/product linking, audit history
START TRANSACTION;
ALTER TABLE users ADD COLUMN IF NOT EXISTS account_status ENUM('active','deactivated','archived') NOT NULL DEFAULT 'active' AFTER is_active;
ALTER TABLE users ADD COLUMN IF NOT EXISTS deactivation_reason VARCHAR(500) NULL AFTER account_status;
ALTER TABLE users ADD COLUMN IF NOT EXISTS deactivated_at DATETIME NULL AFTER deactivation_reason;
ALTER TABLE users ADD COLUMN IF NOT EXISTS deactivated_by INT NULL AFTER deactivated_at;
ALTER TABLE users ADD COLUMN IF NOT EXISTS archived_at DATETIME NULL AFTER deactivated_by;
ALTER TABLE users ADD COLUMN IF NOT EXISTS archived_by INT NULL AFTER archived_at;
ALTER TABLE employees ADD COLUMN IF NOT EXISTS termination_reason VARCHAR(500) NULL AFTER status;
ALTER TABLE po_items ADD COLUMN IF NOT EXISTS product_id INT NULL AFTER po_id;
CREATE TABLE IF NOT EXISTS user_status_history(id INT NOT NULL AUTO_INCREMENT,user_id INT NOT NULL,action ENUM('deactivated','reactivated','archived','restored','permanently_deleted') NOT NULL,reason VARCHAR(500) NULL,performed_by INT NULL,created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY(id),KEY user_id(user_id),KEY performed_by(performed_by)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
INSERT IGNORE INTO permissions(permission_name,module,description) VALUES('view_user_accounts','hrm','View employee user accounts'),('manage_user_accounts','hrm','Manage user accounts'),('archive_users','hrm','Archive and restore accounts'),('permanently_delete_users','hrm','Permanently delete archived accounts'),('adjust_inventory','inventory','Authorized stock adjustments'),('receive_goods','procurement','Receive goods into inventory'),('approve_purchase_orders','procurement','Approve purchase orders'),('manage_invoices','procurement','Manage supplier invoices');
INSERT IGNORE INTO role_permissions(role_id,permission_id) SELECT 1,id FROM permissions;
COMMIT;


-- FINAL ROLE/WORKFLOW HARDENING
START TRANSACTION;
INSERT INTO roles(role_name,description)
SELECT 'Procurement','Manages requisitions, RFQs, suppliers, purchase orders and receiving'
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE role_name='Procurement');

-- HRM
INSERT IGNORE INTO role_permissions(role_id,permission_id)
SELECT r.id,p.id FROM roles r JOIN permissions p
WHERE r.role_name='HRM' AND p.permission_name IN ('view_attendance','manage_attendance','view_user_accounts','manage_user_accounts','archive_users','permanently_delete_users');

-- Cashier
INSERT IGNORE INTO role_permissions(role_id,permission_id)
SELECT r.id,p.id FROM roles r JOIN permissions p
WHERE r.role_name='Cashier' AND p.permission_name IN ('view_products','process_sales','view_sales','view_returns','manage_returns');

-- Inventory
INSERT IGNORE INTO role_permissions(role_id,permission_id)
SELECT r.id,p.id FROM roles r JOIN permissions p
WHERE r.role_name='Inventory' AND p.permission_name IN ('view_products','manage_products','adjust_inventory','receive_goods');

-- Finance
INSERT IGNORE INTO role_permissions(role_id,permission_id)
SELECT r.id,p.id FROM roles r JOIN permissions p
WHERE r.role_name='Finance' AND p.permission_name IN ('view_reports','view_expenses','manage_expenses','view_budget','manage_budget','view_invoices','manage_invoices','manage_payments','approve_purchase_orders','view_performance');

-- Procurement
INSERT IGNORE INTO role_permissions(role_id,permission_id)
SELECT r.id,p.id FROM roles r JOIN permissions p
WHERE r.role_name='Procurement' AND p.permission_name IN ('view_requisitions','create_requisitions','approve_requisitions','view_rfqs','manage_rfqs','view_suppliers','manage_suppliers','view_pos','manage_pos','receive_goods','view_invoices');
COMMIT;


-- USER ACCOUNT APPROVAL WORKFLOW + FINANCE/HRM HARDENING
START TRANSACTION;
...
-- ACCOUNT APPROVAL PERMISSION HARDENING
START TRANSACTION;
INSERT IGNORE INTO permissions(permission_name,module,description)
VALUES('approve_user_account_actions','hrm','Approve or reject account deactivation requests');
INSERT IGNORE INTO role_permissions(role_id,permission_id)
SELECT r.id,p.id FROM roles r JOIN permissions p
WHERE r.role_name IN ('Admin','HRM') AND p.permission_name='approve_user_account_actions';
COMMIT;


-- V3 PROFESSIONAL HARDENING MIGRATION
START TRANSACTION;
ALTER TABLE budgets MODIFY status ENUM('pending_approval','active','closed','rejected') NOT NULL DEFAULT 'pending_approval';
ALTER TABLE budgets ADD COLUMN IF NOT EXISTS created_by INT NULL AFTER created_at;
ALTER TABLE budgets ADD COLUMN IF NOT EXISTS approved_by INT NULL AFTER created_by;
ALTER TABLE budgets ADD COLUMN IF NOT EXISTS approved_at DATETIME NULL AFTER approved_by;
ALTER TABLE budgets ADD COLUMN IF NOT EXISTS rejection_reason VARCHAR(500) NULL AFTER approved_at;

INSERT IGNORE INTO permissions(permission_name,module,description) VALUES
('approve_expenses','finance','Approve or reject expense requests'),
('approve_budgets','finance','Approve or reject budget requests'),('manage_job_postings','hrm','Create and manage job postings'),('view_job_applications','hrm','View job applications');

INSERT IGNORE INTO role_permissions(role_id,permission_id)
SELECT r.id,p.id FROM roles r JOIN permissions p
WHERE r.role_name IN ('Admin','Finance') AND p.permission_name IN ('approve_expenses','approve_budgets');
INSERT IGNORE INTO role_permissions(role_id,permission_id) SELECT r.id,p.id FROM roles r JOIN permissions p WHERE r.role_name IN ('Admin','HRM') AND p.permission_name IN ('manage_job_postings','view_job_applications');
...
CREATE TABLE IF NOT EXISTS job_postings(
 ...
 PRIMARY KEY(id), KEY status(status), KEY created_by(created_by)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS stock_adjustment_requests(
 id INT NOT NULL AUTO_INCREMENT, product_id INT NOT NULL, user_id INT NOT NULL, type ENUM('in','out') NOT NULL,
 quantity INT NOT NULL, reference VARCHAR(100) NULL, reason VARCHAR(500) NOT NULL,
 status ENUM('pending','approved','rejected') NOT NULL DEFAULT 'pending', reviewed_by INT NULL, reviewed_at DATETIME NULL, reviewer_note VARCHAR(500) NULL, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
 PRIMARY KEY(id), KEY product_id(product_id), KEY user_id(user_id), KEY status(status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
INSERT IGNORE INTO permissions(permission_name,module,description) VALUES('approve_stock_adjustments','inventory','Approve or reject stock adjustment requests');
INSERT IGNORE INTO role_permissions(role_id,permission_id) SELECT r.id,p.id FROM roles r JOIN permissions p WHERE r.role_name IN ('Admin','Inventory') AND p.permission_name='approve_stock_adjustments';
COMMIT;
