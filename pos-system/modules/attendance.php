...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_attendance'])) {
    ...
    if (!in_array($status, ['present','late','half-day','absent'], true)) throw new Exception('Invalid attendance status.');
    $notes = sanitize($_POST['notes']);
    $date = date('Y-m-d');
    $time_in = date('H:i:s');
    
    try {
        $u = $db->prepare("SELECT full_name FROM users WHERE id=? AND COALESCE(is_active,1)=1 LIMIT 1");
        $u->execute([$user_id]);
        $activeUser = $u->fetch();
        if (!$activeUser) throw new Exception('Selected employee account is not active or does not exist.');
        $d = $db->prepare('SELECT id FROM attendance WHERE user_id=? AND date=? LIMIT 1');
        $d->execute([$user_id,$date]);
        if ($d->fetch()) throw new Exception('Attendance for this user has already been recorded today.');
