...
function performThreeWayMatch($invoice_id) {
    ...
    $po = $db->prepare('SELECT status FROM purchase_orders WHERE id=?');
    $po->execute([$inv['po_id']]);
    $poStatus = $po->fetchColumn();
    if (!in_array($poStatus, ['fulfilled', 'closed'], true)) {
        $db->prepare("UPDATE invoices SET match_status='pending', status='received' WHERE id=? AND status<>'paid'")
            ->execute([$invoice_id]);
        return 'awaiting_final_receipt';
    }

    // Match against every completed receipt for the PO, not only the first GRN.
    $grnTotal = $db->prepare("SELECT COALESCE(SUM(gri.accepted_quantity * pi.unit_price), 0)
        FROM gr_items gri
        JOIN goods_receipts gr ON gr.id = gri.goods_receipt_id
        JOIN po_items pi ON pi.id = gri.po_item_id
        WHERE gr.po_id = ? AND gr.status = 'completed'");
    $grnTotal->execute([$inv['po_id']]);
    $grnTotalAmount = (float)$grnTotal->fetchColumn();

    $difference = abs((float)$inv['net_amount'] - $grnTotalAmount);
    $tolerance = max((float)$inv['net_amount'] * 0.02, 0.01);
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['run_match'])) {
    try {
        ...
        $result=performThreeWayMatch($invoice_id);
        logActivity("3-way match run for invoice #{$invoice_id}: {$result}");
        header('Location: '.BASE_PATH.'modules/procurement/invoices.php?success='.urlencode('3-way match completed: '.ucfirst($result))); exit();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['create_invoice'])) {
    ...
    if (!$supplier_id) {
        ...
        try {
            ...
            $db->commit();
            $success_message = "Invoice #{$invoice_number} created. Run 3-way match when the invoice is ready for verification.";
            logActivity("Created invoice #{$invoice_number} - Pending 3-way match");
                ...
                <div class="card-body table-responsive">
                    <?php if (empty($invoices)): ?>
                        <div class="alert alert-info">
                            ...
                            <br><small>Invoices are created automatically when accepted goods are posted.</small>
                        </div>
                    <?php else: ?>
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if ($inv['match_status'] === 'pending' || $inv['match_status'] === 'mismatch'): ?>
                                    <form method="POST" class="d-inline"><input type="hidden" name="csrf_token" value="<?php echo h(csrf_token()); ?>"><input type="hidden" name="invoice_id" value="<?php echo (int)$inv['id']; ?>"><button name="run_match" class="btn btn-sm btn-info"><i class="fas fa-link"></i> 3-Way Match</button></form>
                                    <?php endif; ?>
