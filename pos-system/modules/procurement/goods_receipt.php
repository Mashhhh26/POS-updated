...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['receive_goods'])) {
    try {
        ...
        $status = $fullyReceived ? 'fulfilled' : 'partially_received';
        $db->prepare('UPDATE purchase_orders SET status=? WHERE id=?')->execute([$status, $po]);

        // A completed receipt creates (or updates) one system invoice for the PO.
        // Its amount always reflects the accepted goods received so far, allowing
        // partial deliveries without producing duplicate invoices.
        $acceptedTotal = $db->prepare("SELECT COALESCE(SUM(gri.accepted_quantity * pi.unit_price), 0)
            FROM gr_items gri
            JOIN goods_receipts gr ON gr.id = gri.goods_receipt_id
            JOIN po_items pi ON pi.id = gri.po_item_id
            WHERE gr.po_id = ? AND gr.status = 'completed'");
        $acceptedTotal->execute([$po]);
        $acceptedAmount = round((float)$acceptedTotal->fetchColumn(), 2);
        $invoiceNumber = null;
        $invoiceAction = null;

        if ($acceptedAmount > 0) {
            $invoice = $db->prepare('SELECT id, invoice_number, status, notes FROM invoices WHERE po_id=? ORDER BY id ASC LIMIT 1 FOR UPDATE');
            $invoice->execute([$po]);
            $existingInvoice = $invoice->fetch();
            $autoNotePrefix = 'Auto-created from Goods Receipts';

            if (!$existingInvoice) {
                $invoiceNumber = 'AUTO-INV-PO-' . $po;
                $note = $autoNotePrefix . '; latest receipt ' . $grn . '.';
                $db->prepare("INSERT INTO invoices(invoice_number,po_id,supplier_id,invoice_date,total_amount,tax_amount,net_amount,notes,status,match_status)
                    VALUES(?,?,?,CURDATE(),?,0,?,?, 'received','pending')")
                    ->execute([$invoiceNumber, $po, $poData['supplier_id'], $acceptedAmount, $acceptedAmount, $note]);
                $invoiceAction = 'created';
            } elseif (strpos((string)($existingInvoice['notes'] ?? ''), $autoNotePrefix) === 0
                && in_array($existingInvoice['status'], ['received', 'pending_payment'], true)) {
                $invoiceNumber = $existingInvoice['invoice_number'];
                $note = $autoNotePrefix . '; latest receipt ' . $grn . '.';
                $db->prepare("UPDATE invoices SET invoice_date=CURDATE(), total_amount=?, tax_amount=0, net_amount=?, notes=?, status='received', match_status='pending' WHERE id=?")
                    ->execute([$acceptedAmount, $acceptedAmount, $note, $existingInvoice['id']]);
        ...
        $receiptSummary = "Goods received: {$grn}";
        if ($invoiceAction === 'created') {
            $receiptSummary .= " · Invoice {$invoiceNumber} created automatically.";
        } elseif ($invoiceAction === 'updated') {
            $receiptSummary .= " · Invoice {$invoiceNumber} updated from accepted goods.";
        }
        $receiptSummary .= $fullyReceived ? ' · PO fulfilled; it closes after all invoices are paid.' : ' · PO is partially received.';
        logActivity("Received goods for PO #{$po} - {$grn}" . ($invoiceAction ? "; invoice {$invoiceNumber} {$invoiceAction}" : ''));
        header('Location: ' . BASE_PATH . 'modules/procurement/goods_receipt.php?success=' . urlencode($receiptSummary));
        exit();
    } catch (Throwable $e) {
