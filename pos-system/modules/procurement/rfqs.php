...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['evaluate_quotation'])) {
    ...
    require_permission('manage_rfqs');
    ...
    if (!valid_percent($technical_score) || !valid_percent($commercial_score)) {
        echo json_encode(['success' => false, 'message' => 'Evaluation scores must be between 0 and 100.']);
        exit();
    }
        ...
        document.getElementById('evalForm')?.addEventListener('submit', function(e) {
            ...
            .then(async response => {
                ...
                try {
                    ...
                    throw new Error(body.trim() || `Evaluation request failed (HTTP ${response.status}).`);
                }
                if (!response.ok) {
                    throw new Error(payload.message || `Evaluation request failed (HTTP ${response.status}).`);
                }
                return payload;
            })
            .then(data => {
            ...
            .catch(error => {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error!',
                    text: error.message || 'Error submitting evaluation! Please try again.',
                    confirmButtonColor: '#dc3545',
                    confirmButtonText: 'OK'
                });
                btn.innerHTML = originalText;
