...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['process_sale'])) {
    ...
    $customer_name = !empty($_POST['customer_name']) ? sanitize($_POST['customer_name']) : 'Walk-in Customer';
    ...
    try {
        ...
        $stmt = $db->prepare("INSERT INTO sales (invoice_number, user_id, customer_id, customer_name, total_amount, discount, tax, payment_method, status) 
                               VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'completed')");
        $stmt->execute([$invoice_number, $_SESSION['user_id'], $customer_id, $customer_name, $total_amount, $discount, $tax, $payment_method]);
        $sale_id = $db->lastInsertId();
        
        foreach ($items as $item) {
            $stmt = $db->prepare("INSERT INTO sales_items (sale_id, product_id, quantity, unit_price, subtotal) VALUES (?, ?, ?, ?, ?)");
        ...
        $db->commit();
        logActivity("Processed sale #{$invoice_number} - ₱" . number_format($grand_total, 2));
        
        echo json_encode([
            'success' => true, 
            'message' => "Sale #{$invoice_number} completed!",
