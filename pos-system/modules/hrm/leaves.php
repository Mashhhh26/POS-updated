...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_leave'])) {
    ...
    try {
        ...
        $empStmt = $db->prepare("SELECT first_name, last_name, email FROM employees WHERE id = ?");
        $empStmt->execute([$employee_id]);
        $employee = $empStmt->fetch();
        if ($employee && filter_var($employee['email'] ?? '', FILTER_VALIDATE_EMAIL)) {
            $subject = 'HRMS Leave Request Update';
            $body = '<h2>Leave request recorded</h2>'
                  . '<p>Hello ' . h($employee['first_name']) . ',</p>'
                  . '<p>Your <strong>' . h(ucfirst($leave_type)) . '</strong> leave request has been recorded.</p>'
                  . '<p><strong>Schedule:</strong> ' . h($start_date) . ' to ' . h($end_date) . '<br>'
                  . '<strong>Status:</strong> ' . h(ucfirst($status)) . '<br>'
                  . '<strong>Reason:</strong> ' . h($reason) . '</p>';
            send_hr_email($employee['email'], $subject, $body);
        }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_status'])) {
    ...
    $empStmt = $db->prepare("SELECT e.first_name, e.email, l.leave_type, l.start_date, l.end_date, l.reason FROM leaves l JOIN employees e ON e.id = l.employee_id WHERE l.id = ?");
    $empStmt->execute([$id]);
    $leaveInfo = $empStmt->fetch();
    if ($leaveInfo && filter_var($leaveInfo['email'] ?? '', FILTER_VALIDATE_EMAIL)) {
        $body = '<h2>Leave request status updated</h2>'
              . '<p>Hello ' . h($leaveInfo['first_name']) . ',</p>'
              . '<p>Your <strong>' . h(ucfirst($leaveInfo['leave_type'])) . '</strong> leave request is now <strong>' . h(ucfirst($status)) . '</strong>.</p>'
              . '<p><strong>Schedule:</strong> ' . h($leaveInfo['start_date']) . ' to ' . h($leaveInfo['end_date']) . '<br>'
              . '<strong>Reason:</strong> ' . h($leaveInfo['reason']) . '</p>';
        send_hr_email($leaveInfo['email'], 'HRMS Leave Status: ' . ucfirst($status), $body);
    }
