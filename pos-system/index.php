...
(function(){try{var t=localStorage.getItem('pos_theme');var p=localStorage.getItem('pos_palette');if(t==='dark'||t==='light')document.documentElement.setAttribute('data-pos-theme',t);if(['indigo','blue','emerald','violet','rose','amber'].indexOf(p)!==-1)document.documentElement.setAttribute('data-pos-palette',p);}catch(e){}})();
    ...
    <script src="<?php echo BASE_PATH; ?>assets/vendor/chartjs/chart.umd.min.js"></script>
</head>
<body>
    ...
    <div class="modal fade" id="attendanceSummaryModal" tabindex="-1">
      <div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0">
        <div class="modal-header"><div><div class="text-primary small fw-bold">ATTENDANCE</div><h5 class="modal-title mb-0">Today's attendance</h5></div><button class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body"><div class="row g-3">
          <div class="col-6"><div class="small text-muted">Time In</div><h4><?php echo h($hasAttendance['time_in']?:'—'); ?></h4></div>
          <div class="col-6"><div class="small text-muted">Time Out</div><h4><?php echo h($hasAttendance['time_out']?:'—'); ?></h4></div>
        </div><div class="alert alert-info small mb-0 mt-3"><i class="fas fa-circle-info me-1"></i>Your attendance is recorded automatically when you log in and completed when you log out.</div></div>
        <div class="modal-footer"><a href="<?php echo BASE_PATH; ?>modules/attendance.php" class="btn btn-outline-primary">View Attendance</a><button class="btn btn-primary" data-bs-dismiss="modal">Close</button></div>
      </div></div>
    </div>
    <?php endif; ?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded',function(){
          const key='pos_attendance_popup_<?php echo date('Y-m-d'); ?>_<?php echo (int)$_SESSION['user_id']; ?>';
          if(!sessionStorage.getItem(key)){sessionStorage.setItem(key,'1');const el=document.getElementById('attendanceSummaryModal');if(el)new bootstrap.Modal(el).show();}
        });
        <?php endif; ?>
    </script>
</body>
